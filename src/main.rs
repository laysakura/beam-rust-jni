...
fn main() -> Result<(), J4RsError> {
    let entry = ClasspathEntry::new(
        "/Users/sho.nakatani/.ghq/src/github.com/laysakura/beam-rust-jni/word-count-beam/target/word-count-beam-0.1-jar-with-dependencies.jar",
    );
    ...
    let boolean_instance = jvm.invoke(&wordcount_class_instance, "entrypoint", &Vec::new())?;
